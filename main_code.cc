	...
	virtual bool VisitFunctionDecl(FunctionDecl *func) {
		...
		for(int i=0; i<func->getNumParams(); i++) {
			...
			if(!y->getType()->isPointerType()) {
				...
				std::string datatype = QualType::getAsString(y->getType().split(), PrintingPolicy{ {} });
				...
				// if not found in any kind of assignment operation, show warning

				if(!(std::find(assignmentVars.begin(), assignmentVars.end(), arg_name) != assignmentVars.end()))

					printWarning("pass by value argument - "+arg_name+" - possibly not modified!!"+

						" - SUGGESTION: const "+datatype+" *"+arg_name, func);
